...
async def navigate_and_buy(conv):
    """Единый алгоритм навигации и покупки"""
...
async def test_full_cycle():
    """Прогоняет ВСЮ цепочку до упора при запуске"""
    print("--- ПРОВЕРКА ПРИ ЗАПУСКЕ: ПРОГОН ВСЕЙ ЦЕПОЧКИ ---")
    step = 0
    while step < 10: # Защита от бесконечного цикла в тесте
        step += 1
        try:
            async with client.conversation(bot_username, timeout=5) as conv:
                ...
                if status == "bought":
                    print("Тест завершен: Товар успешно пройден и куплен!")
                    break
                elif status == "navigated":
                    print("Тест: Переход выполнен, моментально листаю дальше...")
                    # continue сразу же запустит новую итерацию с /shop
                    continue
                else:
                    print(f"Тест завершен: Дошли до конца, нужных кнопок больше нет.")
                    break
        ...
        try:
            async with client.conversation(bot_username, timeout=3) as conv:
                ...
                if status == "bought": 
                    ...
                    # Если нажали кнопку перехода, не ждем ни миллисекунды,
                    # сразу летим на новую итерацию отправлять /shop
                    continue
                
        except Exception:
            # Ошибки связи игнорим
            pass
        
        # Задержка ТОЛЬКО если нужных кнопок вообще нет (ждем появления товара)
